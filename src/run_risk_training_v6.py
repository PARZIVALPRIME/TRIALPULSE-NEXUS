"""
TRIALPULSE NEXUS 10X - Risk Classifier v6.0 FAST OPTIMIZED
Production-grade patient risk classification - FAST VERSION
- Optimized feature engineering (25+ features)
- Enhanced class weights for minority recall
- Aggressive threshold optimization
- Single-threaded for reliability, reduced iterations for speed

Target: Beat v4 results (67%/44%/48% recall)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(OUTPUT_DIR / 'training_v6.log', mode='w')
    ]
)
log = logging.getLogger(__name__)

# ============================================================================
# CONFIGURATION - AGGRESSIVE WEIGHTS FOR RECALL
# ============================================================================

# More aggressive class weights to boost minority recall
CLASS_WEIGHTS = {
    0: 20.0,  # Critical (was 10 in v5)
    1: 15.0,  # High (was 8 in v5)
    2: 1.0,   # Low (majority)
    3: 8.0,   # Medium (was 4 in v5)
}
...
def create_risk_target(df: pd.DataFrame) -> pd.Series:
    ...
    # CRITICAL indicators (weight ~4-5)
    ...
    # Get percentiles for binning
    p50 = risk.quantile(0.50)
    p80 = risk.quantile(0.80)
    p95 = risk.quantile(0.95)
...
def run_training():
    ...
    print("\n" + "=" * 70)
    print("  TRIALPULSE NEXUS 10X — RISK CLASSIFIER v6 (FAST)")
    print("=" * 70)
    print(f"  Started: {start.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70 + "\n")
    
    log.info("=" * 60)
    log.info("RISK CLASSIFIER v6 FAST — Training Started")
    log.info("=" * 60)
    ...
    # Random Forest - REDUCED ITERATIONS for speed
    ...
    if XGB_AVAILABLE:
        ...
        xgb_w = weights.copy()
        xgb_w[y_train == 0] *= 3.0  # More aggressive Critical boost
        xgb_w[y_train == 1] *= 2.5  # More aggressive High boost
        xgb_w[y_train == 3] *= 2.0  # Medium boost
    ...
    # LightGBM - Use 'balanced' instead of dict to avoid KeyError with SMOTE
    if LGB_AVAILABLE:
    ...
    models['GB'].fit(X_train, y_train, sample_weight=weights)
    ...
    for name, model in models.items():
        ...
        log.info(f"    AUC: {auc:.4f} | Targets: {targets_met}/4")
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / 'tables' / 'production_metrics_v6.csv', index=False)
    
    # Save models
    for name, model in models.items():
        with open(OUTPUT_DIR / 'models' / f'model_v6_{name.lower()}.pkl', 'wb') as f:
            pickle.dump(model, f)
    
    # Save scaler and encoder
    with open(OUTPUT_DIR / 'models' / 'scaler_v6.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    with open(OUTPUT_DIR / 'models' / 'label_encoder_v6.pkl', 'wb') as f:
        pickle.dump(le, f)
    ...
    with open(OUTPUT_DIR / 'models' / 'production_config_v6.json', 'w') as f:
    ...
    ax.set_title(f'{best_name} Confusion Matrix — v6', fontweight='bold')
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / 'figures' / 'confusion_matrix_v6.png', dpi=150)
    plt.close()
    ...
    ax.set_title('Per-Class Recall — Risk Classifier v6', fontweight='bold')
    ax.legend()
    ax.set_ylim(0, 1.1)
    ax.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / 'figures' / 'per_class_recall_v6.png', dpi=150)
    plt.close()
    
    # Training report
    with open(OUTPUT_DIR / 'training_report_v6.json', 'w') as f:
        json.dump({
            'version': '6.0.0',
            'created': datetime.now().isoformat(),
    ...
    print("\n" + "=" * 70)
    print("  ✅ TRAINING COMPLETE — RISK CLASSIFIER v6")
    print("=" * 70)
